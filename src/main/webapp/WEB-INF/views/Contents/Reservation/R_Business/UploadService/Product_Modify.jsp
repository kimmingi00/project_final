  ...
  <style type="text/css">
	...
	<section style="margin-top:26%;">	
        <div class="box">
                ...
                <br>
                <form name="member" method="post" enctype="multipart/form-data" action="ReservServlet?command=ProductModifyPro&b_id=${ b_user.b_id }&p_idx=${ p_idx }">
                  <div class="inputBox">
	              ...
	              <div class="inputBox" >
                  	...
                  	<select name="place" style="height:30px; margin-left: 105px;    ">
                  		...
                  		<option value="1" <c:if test="${ place == 1 }">selected</c:if>>ソウル/京畿/仁川</option>
                  		<option value="2" <c:if test="${ place == 2 }">selected</c:if>>江原道</option>
                  		<option value="3" <c:if test="${ place == 3 }">selected</c:if>>大田/忠清南道/忠淸北道</option>
                  		<option value="4" <c:if test="${ place == 4 }">selected</c:if>>光州/全羅南道/全羅北道</option>
                  		<option value="5" <c:if test="${ place == 5 }">selected</c:if>>釜山/大邱/慶尙南道/慶尙北道</option>
                  	</select>
                  </div>
	              
	             
	              <c:if test="${ p_in_cnt ==0 }">
	              
	              <div class="inputBox" id="p_in" style=" margin-top: 30px;">
                          <input type="text" name="p_in" style="width:180px;" required onkeyup="this.setAttribute('value', this.value);" value="${ p_in }">
                  ...
                  <div class="inputBox" >
	                  <input type="text" name="p_indate" id="datepicker1" style="width:180px;" required onkeyup="this.setAttribute('value', this.value);"  value="${pvo.p_indate.substring(0,10) }">
	                  <label><font size="4">出発日</font></label>
	                  <input type="time" name="p_intime" style="width:180px;" value="${ p_intime }">
	              </div>
	              </c:if>
	             
	              <c:if test="${ p_in_cnt !=0 }">
	              <c:forEach var="p_in" items="${ p_in }" begin="0" end="0" varStatus="stat">
	              
	              <div class="inputBox" id="p_in" style=" margin-top: 30px;">
                          <input type="text" name="p_in" style="width:180px;" required onkeyup="this.setAttribute('value', this.value);" value="${ p_in }">
                          <label><font size="4">搭乗地</font></label>
                          <input type="hidden" name="cnt" value="${ stat.count+1 }">
                          <input type="button" value="地域追加"  style="WIDTH: 95pt; HEIGHT: 30pt; font-size : 16px" onclick="add_row(member.cnt.value)">
                  </div>
                   <c:forEach var="p_intime" items="${ p_intime }" begin="0" end="0">
                  <div class="inputBox" >
	                  <input type="text" name="p_indate" id="datepicker1" style="width:180px;" required onkeyup="this.setAttribute('value', this.value);"  value="${ pvo.p_indate }">
	                  <label><font size="4">出発日</font></label>
	                  <input type="time" name="p_intime" style="width:180px;" value="${ p_intime }">
	              </div>
	              </c:forEach>
				...
				<c:if test="${ p_in_cnt !=0 }">
	              <c:forEach var="p_in" items="${ p_in }" begin="1" varStatus="stat">

                  <table>
                    <tbody id="add_p_in" class="inputBox">
						<tr>
							<td>
								<input type="text" name="p_in${ stat.index+1 }" style="width:180px;" required="" onkeyup="this.setAttribute(" value',="" this.value);'="" value="${ p_in }">
							 <c:forEach var="p_intime" items="${ p_intime }" begin="${ stat.index }" end="${ stat.index }">
								<input type="time" name="p_intime${ stat.index+1 }" style="width:100px;" value="${ p_intime }">
							 </c:forEach>	
								<input type="button" value="X" style="WIDTH: 95pt; HEIGHT: 30pt; font-size : 16px" onclick="delete_row(member.cnt.value)">
							</td>
						</tr>	
                    </tbody>
                  </table>
	              ...
	              <div class="inputBox">
                  	...
                  	<select name="outplace" style="height:30px; margin-left: 105px;">
                  		<option value="0" >地域選択</option>
                  		<option value="1" <c:if test="${ outplace == 1 }">selected</c:if>>ソウル/京畿/仁川</option>
                  		<option value="2" <c:if test="${ outplace == 2 }">selected</c:if> >江原道</option>
                  		<option value="3" <c:if test="${ outplace == 3 }">selected</c:if>>大田/忠清南道/忠淸北道</option>
                  		<option value="4" <c:if test="${ outplace == 4 }">selected</c:if>>光州/全羅南道/全羅北道</option>
                  		<option value="5" <c:if test="${ outplace == 5 }">selected</c:if>>釜山/大邱/慶尙南道/慶尙北道</option>
                  	</select>
                  ...
                  <div class="inputBox">
                        ...
                        <input type="time" name="p_outtime" style="width:180px;" value="${ pvo.p_outtime }">
                  </div>
                  ...
                  <div class="inputBox">
                        <input type="text" name="p_spot" style="width:180px;" required onkeyup="this.setAttribute('value', this.value);" value="${ pvo.p_spot }">
                        <label><font size="4">観光地搭乗地</font></label>
                        <input type="time" name="p_spottime" style="width:180px;" value="${ pvo.p_spottime }">
                  </div>

                  <div class="inputBox">
                        <input type="text" name="p_stopover" style="width:180px;" required onkeyup="this.setAttribute('value', this.value);" value="${ pvo.p_stopover }">
                        <label><font size="4">下車地</font></label>
                        <input type="time" name="p_stoptime" style="width:180px;" value="${ pvo.p_stoptime }">
                  </div>
                  ...
                  <div class="thema">
                    ...
                    <span style="margin:60px;"><input type="checkbox" name="thema" value="1">食事可否</span>
                     ...
                     <div class="inputBox" id="p_file">
                        ...
                        <input type="text" name="filename_origin" value="${ pvo.p_filename }">
                  </div>
