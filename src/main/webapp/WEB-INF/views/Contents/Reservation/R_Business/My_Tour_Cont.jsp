...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
 ...
 <link rel="stylesheet"  href="/Contents/Index/imgslider_css/lightslider.css"/>
	...
	<div class="view_title">
		...
		<h2 style="color:white;position: absolute;left: 28%;top: 50%;font-size: 50px; text-shadow: 2px 1px 1px black;">今, 여기관광と一緒に旅行してみませんか？</h2>
	</div>
	<div class="flash">
	...
	<section class="product">
		<div class="cont_img">
			<c:if test="${ pvo.p_filename==null }">
				<img src="/Contents/img/product_img/no-image.png" style="width:650px; height:340px; ">
			</c:if>
			<c:if test="${ pvo.p_filename!=null }">			
			    <div class="demo">
			        <div class="item">
		...
		<c:if test="${ (indateNum-sysDateNum) <= 0 }">	
			...
			<form name="fm" method="post">
				<c:if test="${ pvo.p_reservpeople<pvo.p_maxpeople }">
					...
					<c:if test="${ (indateNum-sysDateNum) > 0 }">
						...
						<c:if test="${ row == 0 || !empty cnt}">
							...
							<a href="/Contents/Reservation/Reserv_unregitmodify.jsp">非会員 予約変更 / 取り消し</a>
						</c:if>
						...
						<c:if test="${ row == 1 }">
							...
							<a href="ReservServlet?command=reserv_modify&p_idx=${ pvo.p_idx }&id=${ user.id }"><input type="button" value="予約変更 / 取り消し" class="btn"></a>
						</c:if>
				...
				<a href="/ReservServlet?command=ProductModify&idx=${ pvo.p_idx }"><input type="button" name="modi" value="修整" class="btn"></a>&nbsp;&nbsp;&nbsp;
				<a href="/ReservServlet?command=ProductDelete&idx=${ pvo.p_idx }&id=${b_user.b_id}"><input type="button" name="delete" value="削除" class="btn"></a>
